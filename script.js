//input set up as a template literal so that the multi-line string input required is allowed
const input = 
`zlmlk1
...
//tracks line number for final array of the numbers located in each line
let arrayIndex = 0;

//array that will store the numbers found from each line
let numbersArray = []

//stores the numbers that wil be used in the final calculation
let sumArray = []

splitlines.forEach(line => {
    //string to hold numbers found in each line
    for (let character = 0; character < line.length; character++) {
        //stores the current character in the line the loop is up to
        let currentCharacter = line[character];
        //checks if current character is a number
        if (!isNaN(currentCharacter)) {
            //records the first number into a string
            let foundNumbers = currentCharacter
            //now reverses the search to start from the end of the string to find the last number present
            for (let searchIndex = line.length -1; searchIndex > -1; searchIndex--) {
                let currentSearchCharacter = line[searchIndex]
                if(!isNaN(currentSearchCharacter)) {
                    foundNumbers+= currentSearchCharacter
                    //exits the loop once the second number has been located
                    break
                }
            }
            
            //adds the numbers found in the line into an array for use in calculating the total later
            sumArray.push(foundNumbers)
            //ends the search once the first and last numbers in the line have been located
            break
        }
    }
});
...
//for loop to add all the numbers in the array together to get the final result

let result = 0

for (let index = 0; index < sumArray.length; index++) {
    let number = parseInt(sumArray[index])
